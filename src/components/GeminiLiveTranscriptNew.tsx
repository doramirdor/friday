...
export default function GeminiLiveTranscriptNew() {
  const {
    isAvailable,
    isRecording,
    transcript,
  ...
  const handleStartStop = async () => {
    if (isRecording) {
      ...
      await startRecording({
        ...
        chunkDurationMs: parseInt(chunkDuration),
      });
    }
  };
  ...
  if (!isAvailable) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Gemini Live Transcript</CardTitle>
          <CardDescription>
            Gemini Live Transcript is not available. Please check that you have a valid Gemini API key configured.
          </CardDescription>
        </CardHeader>
      </Card>
    );
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Real-time speech transcription using Google's Gemini AI (2 second chunks)
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="chunk-duration">Chunk Duration (ms)</Label>
              <Select value={chunkDuration} onValueChange={setChunkDuration} disabled={isRecording}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="1000">1 second (very responsive)</SelectItem>
                  <SelectItem value="2000">2 seconds (recommended)</SelectItem>
                  <SelectItem value="3000">3 seconds (more accurate)</SelectItem>
                  <SelectItem value="5000">5 seconds (best accuracy)</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
